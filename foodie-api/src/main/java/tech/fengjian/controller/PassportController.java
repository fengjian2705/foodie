...
public class PassportController {
    ...
    public JSONResult regist(@RequestBody UserBO userBO,
        ...
        // 1. 查询用户名是否存在
        boolean isExist = this.userService.queryUserNameIsExist(username);
        if (isExist) {
            return JSONResult.errorMsg("用户名已存在");
        }
        // 2. 密码长度不能少于6位
        ...
        // 4. 实现注册
        Users userResult = this.userService.createUser(userBO);
    ...
    public JSONResult login(@RequestBody UserBO userBO,
        ...
        // 1. 实现登录
        Users userResult = this.userService.queryUserForLogin(username,
